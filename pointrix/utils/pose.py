...
def se3_exp_map_no_rot(log_transform: torch.Tensor, eps: float = 1e-4) -> torch.Tensor:
    ...
    transform[:, :3, :3] = torch.eye(
        3, dtype=log_transform.dtype, device=log_transform.device)[None]
    transform[:, :3, 3] = T
    transform[:, 3, 3] = 1.0
...
def _se3_V_matrix(
    ...
    V = (
        torch.eye(3, dtype=log_rotation.dtype,
                  device=log_rotation.device)[None]
        + log_rotation_hat
